...
export default async function MyCoursesPage() {
  ...
  // 2. Fetch enrollments from Supabase A
  ...
  // 3. If there are active enrollments, fetch corresponding posts
  if (activeSlugs.length > 0) {
    ...
    if (posts && posts.length > 0) {
      // Map granted date (created_at from enrollment) to the post object
      const slugToEnrollment = new Map(
        enrollments?.map(e => [String(e.course_slug || '').trim(), e])
      );

      courses = posts.map(post => {
        const enrollment = slugToEnrollment.get(String(post.course_slug || '').trim());
        return {
          ...post,
  ...
  // 4. If logged in but has no enrollments
  if (courses.length === 0) {
  ...
  // 5. Render client portal
  return (
    <main className={styles.container}>
      <MyCoursesClient email={email} courses={courses} />
    </main>
  );
}
