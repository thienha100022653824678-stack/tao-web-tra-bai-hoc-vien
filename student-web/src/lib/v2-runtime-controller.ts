...
//
// Runtime active-mode controller for the V1/V2 coexistence platform (Portal port).
//
// TypeScript port of the LMS `utils/v2-runtime-controller.js`. Single source of
// truth for whether the platform is serving V1 or V2 behavior. The mode is
// persisted in Supabase B `site_config` (keys `v2_active_mode` + `v2_kill_switch`)
// so the owner can flip it from the admin UI without a redeploy and without
// touching V1 code.
//
...
//
// Fail-closed for mode RESOLUTION: when the DB is unreadable, the config row
//   is missing, or `lmsSupabaseAdmin` is not configured, the snapshot is set
//   to activeMode='v1' (safe state). The gate therefore forces V1 when the DB
//   is down. Cold-cache fail-open only applies BEFORE the first snapshot is
//   loaded in an instance.
//
...
// If `lmsSupabaseAdmin` is null (not configured), fail-closed to v1. The
// Portal must never fall back to v2 when the LMS DB is unreachable.
function loadSnapshotFromNotConfigured(): RuntimeSnapshot {
...
async function loadSnapshotFromDb(): Promise<RuntimeSnapshot> {
  const stubDb = (globalThis as Record<string, unknown>).__V2_RUNTIME_STUB_DB__ as unknown;
  if (stubDb !== undefined) {
  ...
  // Fail-closed when the LMS Supabase admin client is not configured. This
  // is the safe state — V1 behavior, no V2 features.
  if (!lmsSupabaseAdmin) {
    return loadSnapshotFromNotConfigured();
  }

  try {
    const { data, error } = await lmsSupabaseAdmin
      .from('site_config')
      .select('key, value')
    ...
    for (const row of (data || []) as SiteConfigRow[]) {
...
function recordUpsertToStubDb(key: string, value: unknown): void {
  const stubDb = (globalThis as Record<string, unknown>).__V2_RUNTIME_STUB_DB__ as Record<string, unknown> | undefined;
  if (stubDb && typeof stubDb === 'object') {
    try {
      stubDb[key] = value;
    } catch {
...
async function resolveSnapshot({ forceRefresh = false } = {}): Promise<RuntimeSnapshot> {
  const override = resolveEnvOverride();
  if (override) {
    setCachedSnapshot(override, getCacheTtlMs());
    return { ...override };
  }

  const stub = (globalThis as Record<string, unknown>).__V2_RUNTIME_CONTROLLER_SNAPSHOT__ as
    | RuntimeSnapshot
 ...
 * error / missing config / unconfigured lmsSupabaseAdmin. Async only on the
 * first read in a cold instance; cached afterwards.
 */
 ...
 *   | 'env_force_mode' | 'env_force_kill' | 'stub' | 'stub_error'
...
export type SetActiveModeResult =
  | { ok: true; activeMode: 'v1' | 'v2' }
  | { ok: false; code: 'invalid_mode' | 'db_error' | 'db_exception' };
 ...
 */
export async function setActiveMode(mode: unknown): Promise<SetActiveModeResult> {
  ...
  if ((globalThis as Record<string, unknown>).__V2_RUNTIME_STUB_DB__ !== undefined) {
  ...
  try {
    const { error } = await lmsSupabaseAdmin
      .from('site_config')
      .upsert(
...
export type SetKillSwitchResult =
  ...
  | { ok: false; code: 'db_error' | 'db_exception' };
 ...
 */
export async function setKillSwitch(enabled: unknown): Promise<SetKillSwitchResult> {
  const value = parseBooleanFlag(enabled);
  if ((globalThis as Record<string, unknown>).__V2_RUNTIME_STUB_DB__ !== undefined) {
