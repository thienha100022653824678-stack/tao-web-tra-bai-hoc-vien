...
//
// TS port of web-lms-chinh-thuc/utils/v2-runtime-cache.js.
//
// Synchronous, in-process cache for the resolved V2 runtime mode. Kept in a
// tiny standalone module (no imports) so both `v2-flags.ts` (which the
// behavioral flag readers import) and `v2-runtime-controller.ts` (which does
// the async DB read) can share it WITHOUT a circular import.
//
...
//
// The cache is populated by warmRuntimeConfig() / refreshRuntimeConfig() in
// v2-runtime-controller.ts, which the request routers await once at the top of
// every request. A short TTL keeps one request consistent and avoids hammering
// the DB; the admin flip endpoint calls refresh() so the new mode is visible on
// that instance immediately, and other instances pick it up within the TTL.
